 ...
 */
package eu.serco.dhus.plugin.sral;
...
public class SralPlugin implements Processable
{
	private static Log logger = LogFactory.getLog(SralPlugin.class);
            ...
            if (map.containsKey(productType)) {
                ...
                for(Request aux_request : requestList) {
	                ...
	                for(InputRequest inReqs : requestsMap) {
	                	...
	                	tasks.add(new RequestTask(inReqs.getUrl(), inReqs.getType(), inReqs.getRetrievalMode(), hashedString, SralPlugin.executor));
