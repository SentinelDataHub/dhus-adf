 ...
 */
package eu.serco.dhus.plugin.synergy1eo;
...
public class Synergy1EoPlugin implements Processable
{
	private static Log logger = LogFactory.getLog(Synergy1EoPlugin.class);
            ...
            if (map.containsKey(productType)) {
                ...
                for(Request aux_request : requestList) {
	                ...
	                for(InputRequest inReqs : requestsMap) {
	                	...
	                	tasks.add(new RequestTask(inReqs.getUrl(), inReqs.getType(), hashedString, Synergy1EoPlugin.executor));
				...
				for(Task task : taskTables.get(productType).getTasks()) {
					...
					for(Input in : task.getInputs()) {
						...
						for (Alternative alternative : in.getAlternatives()){
							...
							String url = propFiles.get(productType).getProperty(alternative.getRetrievalMode().trim());
